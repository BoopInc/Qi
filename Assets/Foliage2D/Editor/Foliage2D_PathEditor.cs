...
namespace Foliage
{
    ...
    {
        private Texture2D texDot = GetGizmo("circle.png");
        private Texture2D texBlueDot = GetGizmo("blue_circle.png");
        private Texture2D texDotSelected = GetGizmo("circle_selected.png");
        private Texture2D texDotPlus = GetGizmo("dot-plus.png");
        private Texture2D texMinus = GetGizmo("dot-minus.png");
        ...
        private void UpdateHandles(Foliage2D_Path path2D, GUIStyle iconStyle)
        {
            ...
            for (int i = 0; i < path2D.handlesPosition.Count; i++)
            {
                ...
                global = Handles.FreeMoveHandle(pos, Quaternion.identity, HandleScale(pos), Vector3.zero, Handles.CircleCap);

                if (!path2D.uniformValues && path2D.foliagePathType == Foliage2D_PathType.Smooth)
                    tGlobal = Handles.FreeMoveHandle(tPos, Quaternion.identity, HandleScale(tPos), Vector3.zero, Handles.CircleCap);
                ...
                if (i + 1 < path2D.handlesPosition.Count)
                {
                    ...
                    if (Handles.Button(mid, SceneView.lastActiveSceneView.camera.transform.rotation, handleScale, handleScale, Handles.CircleCap))
                    {
                ...
                if (Event.current.alt && path2D.handlesPosition.Count > 2)
                {
                    ...
                    if (Handles.Button(pos, SceneView.lastActiveSceneView.camera.transform.rotation, handleScale, handleScale, Handles.CircleCap))
                    {
                        if (!isSelected)
                        {
                            selectedPoints.Clear();
